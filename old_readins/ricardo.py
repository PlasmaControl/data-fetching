...
###### USER INPUTS ######
#                       #
shots = [154764,163303,175970] #175970 is Colin's good modulation shot; 154764 is David Eldon's

tmin = 0
tmax = 6000

output_file='ricardo.pkl'
...
cer_channels={'TANGENTIAL': np.arange(9,24+1), #np.arange(1,33),
              'VERTICAL': np.array([])} #np.arange(1,49)}
...
fit_functions_1d=['linear_interp_1d', 'mtanh_1d','csaps_1d']
fit_functions_2d=['nn_interp_2d','linear_interp_2d','rbf_interp_2d']

trial_fits=['csaps_1d'] #'linear_interp_1d','spline_1d'] #,'mtanh_1d']
...
for i in range(len(records)):
    ...
    final_data[shot]={}
    final_data[shot]['t_ip_flat']=np.array(966.0)
    final_data[shot]['ip_flat_duration']=np.array(4987.5)
    ...
    # accomodate the old code's bug of flipping top and bottom 
    '''
...
ricardo_data={}
sigs=set(final_data[shots[0]].keys())-set(['errors','t_ip_flat','ip_flat_duration','shot','rhovn'])
for sig in sigs:
    ricardo_data[sig]=[]
    ricardo_data['shot']=[]
for shot in shots:
    for sig in sigs:
        try: 
            ricardo_data[sig].extend(final_data[shot][sig])
        except:
            print(shot)
            print(sig)
            ricardo_data[sig].extend(np.ones((len(final_data[shot]['time']),len(standard_x)))*np.nan)
    ricardo_data['shot'].extend([shot]*len(final_data[shot]['time']))
for sig in sigs:
    ricardo_data[sig]=np.array(ricardo_data[sig])
    ricardo_data['shot']=np.array(ricardo_data['shot'])
#for i in range(len(ricardo_data['time'])):
#    if np.any([np.nan in ricardo_data[sig] for sig in sigs]):
        
        
#import pdb; pdb.set_trace()
io.savemat('ricardo.mat',ricardo_data)
