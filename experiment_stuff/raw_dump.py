...
all_shots=np.load('new_shots_2.npy')
#all_shots=[163303,187076]
#all_shots=[187076]
#all_shots = [163303] #[185769] #[163303,175970] #175970 is Colin's good modulation shot; 154764 is David Eldon's

tmin = 0
tmax = 6000

output_file_base='/cscratch/abbatej/raw_read/raw_nb_data'
...
for beam in ['30','15','21','33']:
    for direction in ['L','R']:
        for sig in ['pinj','tinj']:
            nb_sig_names.append(r'\{}_{}{}'.format(sig,beam,direction))
...
include_psirz=False #True
include_rhovn=False #True
...
cer_sig_names=[] #['temp','rot','rotc']
cer_scale={'temp': 1e3, 'rot': 1, 'rotc': 1}
cer_type='CERQUICK'
...
# number of chunks of shots we want to dump - get these values via OMFIT with
# for i in range(2008,2022):
    # query='''
    # SELECT shot
    #   FROM summaries
    #  WHERE YEAR(time_of_shot) >= []
    #    AND YEAR(time_of_shot < {}
    # '''.format(i,i+1)

    # sig_info=OMFITrdb(query=query,
    #                   db='d3drdb',
    #                   server='d3drdb',
    #                   by_column=True)

    # my_shots=[int(shot) for shot in sig_info['shot']]
    # print(min(my_shots))
    # print(i)
...
all_shots=np.sort(all_shots)
for i,campaign_name in enumerate(campaign_names):
    print('Starting campaign {}'.format(campaign_name))
    shots=all_shots[np.logical_and(all_shots>first_shot_of_year[i],all_shots<first_shot_of_year[i+1])]
    if len(shots)==0 or (campaign_name!='2013' and campaign_name!='2011'):
        continue
    ...
    def change_timebase(record):
        ...
        for sig_name in []: #nb_sig_names:
            try:
                # we'll average down to 1 ms for signals that are super refined like pinj, lest we save a bunch of data
                # for no reason - plus toksearch crashes 
                average_window=1
                new_times=np.arange(min(record['{}_full'.format(sig_name)]['times']),
                                                            max(record['{}_full'.format(sig_name)]['times']),
                                                            average_window)
                record['{}_full'.format(sig_name)]['data']=standardize_time(record['{}_full'.format(sig_name)]['data'],
                                                                            record['{}_full'.format(sig_name)]['times'],
                                                                            new_times,
                                                                            window_size=average_window) #record['standard_time'])
                record['{}_full'.format(sig_name)]['times']=new_times
            except:
    ...
    for i in range(len(records)):
        ...
        for sig in record.keys():
            if record[sig] is not None:
                final_data[shot][sig]=record[sig]

    with open('{}_{}.pkl'.format(output_file_base,campaign_name),'wb') as f:
